...
# ---------------------------------------------------------
class Student:
    def __init__(self, name, jp, math, eng):
        self.name = name
    ...
    def calculate_average(self):
        return round(self.calculate_total() / 3, 1) # 小数第1位まで
...
def add_student():
    ...
    # エラーチェック（空欄がないか）
    if not name or not jp or not math or not eng:
        messagebox.showerror("エラー", "全ての項目を入力してください")
        return

    try:
        # ★ここでクラスから「インスタンス（実体）」を作成！★
        new_student = Student(name, jp, math, eng)
        
        # クラスのメソッドを使って計算
        total = new_student.calculate_total()
        avg = new_student.calculate_average()
...
# 国語
tk.Label(input_frame, text="国語:").grid(row=0, column=2)
entry_jp = tk.Entry(input_frame, width=5)
entry_jp.grid(row=0, column=3, padx=5)

# 数学
tk.Label(input_frame, text="数学:").grid(row=0, column=4)
entry_math = tk.Entry(input_frame, width=5)
entry_math.grid(row=0, column=5, padx=5)

# 英語
tk.Label(input_frame, text="英語:").grid(row=0, column=6)
...
# 表（Treeview）の作成
columns = ("Name", "JP", "Math", "Eng", "Total", "Avg")
